...
ROOK_DIR = ((1,0),(0,1),(0,-1),(-1,0))                                  # List of all rook vectorial possible moves
...
def is_in_grid(posx,posy):
    ...
    condition=False
    if 0<=posx<NBR_SQU and 0<=posy<NBR_SQU:
        condition=True
...
def move_pawn(G):
    """Stores all the selectedPiece validSquares (pawn type)"""

    pass


def move_rook(G):
    """Stores all the selectedPiece validSquares (rook type)"""
...
def move_knight(G):
    """Stores all the selectedPiece validSquares (knight type)"""

    pass


def move_bishop(G):
    """Stores all the selectedPiece validSquares (bishop type)"""

    pass


def move_queen(G):
    """Stores all the selectedPiece validSquares (queen type)"""

    pass


def move_king(G):
    """Stores all the selectedPiece validSquares (king type)"""
...
def display_valid_squ(G):
    ...
    if G.selectedSquare:
        for i in range(len(G.validSquares)):
            ...
            affiche_rectangle_plein(Point(ABS*LEN_CASE,ORD*LEN_CASE), Point((ABS+1)*LEN_CASE,(1+ORD)*LEN_CASE),ACTIVE_SQU_COLOR)
