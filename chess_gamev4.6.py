...
def are_same_color(G, ABS, ORD):
    ...
    return what_is_piece(G, G.selectedSquare[0], G.selectedSquare[1])[0] == G.grid[ABS][ORD][0]
...
def is_in_check(G, SQUARE_KING):
    """Returns whether king is in check"""

    DIR_LIST = MOVE_LIST[KING] # Stores king move list (vectors)

    KING_COLOR = G.grid[SQUARE_KING[0]][SQUARE_KING[1]] # Stores what color king is
...
def valid_square(G, ABS, ORD):
    ...
    PIECE = G.grid[ABS][ORD] # Stores selectedPiece color/type
...
def move_piece(G):
    ...
    for LST_INDEX in DIR_LIST:
        move_dir(G, ABS, ORD, LST_INDEX, LOOP)


def move_dir(G, ABS, ORD, LST_INDEX, LOOP):
...
def move_king(G):
    """Stores all the king validSquares : Moving the king to different places to test if he is in check"""
    print("fzgg")
    ORIGINAL_KING = G.selectedSquare # Square where is the selected King
    DIR_LIST = MOVE_LIST[KING] # King's movelist

    # See what squares the king is able to go without check in count
    for LST_INDEX in DIR_LIST:
        move_dir(G, ORIGINAL_KING[0], ORIGINAL_KING[1], LST_INDEX, False)


    KING_VALID_SQU = G.validSquares # Saving the Original King validSquares


    for VALID_POS in KING_VALID_SQU: # For all king validPos

        if is_in_check(G, VALID_POS) : # If king is in check on this square

            KING_VALID_SQU.remove(VALID_POS) # Remove square from list



    G.validSquares = KING_VALID_SQU # The validSquares is now the King validSquares without all wrong positions
    G.selectedSquare = ORIGINAL_KING # reselect King
