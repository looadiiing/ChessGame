...
def affiche_rectangle(point1,point2,couleur,epaisseur = 1):
    ...
    point_left_top = Point(min(point1.x,point2.x), max(point1.y,point2.y))
    point_left_top = __pyPoint(point_left_top)
    largeur = max(point1.x,point2.x) - min(point1.x,point2.x)
    hauteur = max(point1.y,point2.y) - min(point1.y,point2.y)
...
def affiche_arc_cercle(centre,rayon,angle_debut,angle_fin,couleur,epaisseur=1):
    ...
    """
    point_top_left = Point(centre.x-rayon,centre.y+rayon)
    point_top_left = __pyPoint(point_top_left)
    rectangle = pygame.Rect(point_top_left,(2*rayon,2*rayon))
...
def affiche_image(nom_image,dest_bas_gauche, source_bas_gauche = None, source_haut_droit = None):
    ...
    if source_bas_gauche and source_haut_droit:
        source_haut_gauche = (source_bas_gauche.x,image.get_height()-source_haut_droit.y)
        hauteur = source_haut_droit.y-source_bas_gauche.y
        area = pygame.Rect(source_haut_gauche,(source_haut_droit.x-source_bas_gauche.x,hauteur))
    dest_haut_gauche = Point(dest_bas_gauche.x,dest_bas_gauche.y+hauteur)
    dest_haut_gauche = __pyPoint(dest_haut_gauche)
...
def affiche_texte(texte,position,couleur,taille_police = __POLICE_SIZE,police = __POLICE_NAME):
    ...
    position = Point(position.x,position.y+myfont.size(texte)[1])
...
def last_clic():
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT or (event.type == KEYDOWN and event.key == K_ESCAPE):
            ...
            exit()
        if event.type == MOUSEBUTTONDOWN and event.button == __MOUSE_LEFT:
           last_clic.x,last_clic.y = pygame.mouse.get_pos()
        if event.type == MOUSEBUTTONUP and event.button == __MOUSE_LEFT:
            is_clic = True
    if is_clic:
        return Point(last_clic.x,__H - last_clic.y)
    else:
        return None
